        ...
        <div className="space-y-24">
          {displayProjects.map((project, index) => (
            <div key={index} className={`relative flex flex-col md:flex-row items-center gap-8 ${index % 2 === 1 ? 'md:flex-row-reverse' : ''}`}>
              {/* Project Image */}
              <div className="w-full md:w-3/5 relative group overflow-hidden rounded-2xl shadow-2xl">
                <img 
                  src={project.image} 
                  alt={project.title} 
                  loading="lazy"
              ...
              <div className={`w-full md:w-2/5 flex flex-col ${index % 2 === 1 ? 'md:items-start text-left' : 'md:items-end md:text-right'} relative z-20`}>
                ...
                <div className="bg-card p-6 rounded-2xl border border-border shadow-xl mb-6 relative z-30 w-full md:w-[120%] md:-ml-[20%]">
                  <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed font-medium">
                    {project.description}
                  </p>
                </div>

                <ul className={`flex flex-wrap gap-4 text-slate-500 dark:text-slate-400 font-mono text-sm mb-6 ${index % 2 === 1 ? 'justify-start' : 'justify-start md:justify-end'}`}>
                ...
                <div className="flex gap-4">
                  {project.github && (
                    <a href={project.github} target="_blank" rel="noopener noreferrer" className="text-slate-500 dark:text-slate-400 hover:text-primary transition-colors">
                      <GitBranch size={20} />
                    </a>
                  )}
                  {project.link && (
                    <a href={project.link} target="_blank" rel="noopener noreferrer" className="text-slate-500 dark:text-slate-400 hover:text-primary transition-colors">
